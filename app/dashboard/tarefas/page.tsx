...
export default function TarefasPage() {
  ...
  useEffect(() => {
    if (!empresaId) return
    const ch = supabase.channel('tarefas-page').on('postgres_changes', { event: '*', schema: 'public', table: 'hub_tarefas', filter: `empresa_id=eq.${empresaId}` }, () => { void carregar() }).subscribe()
  ...
  const totais = { concluida: tarefas.filter(t => t.status === 'concluida').length, executando: tarefas.filter(t => t.status === 'executando').length, custo: tarefas.reduce((s, t) => s + Number(t.custo_usd ?? 0), 0) }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        {['', 'concluida', 'executando', 'erro'].map(s => (
          <button key={s} onClick={() => setFiltroStatus(s)} className={`btn btn-sm ${filtroStatus === s ? 'btn-primary' : 'btn-ghost'}`}>
            {s || 'Todos'}
          </button>
        ))}
        <span style={{ marginLeft: 'auto', fontSize: 12, color: 'var(--text-muted)', display: 'flex', alignItems: 'center' }}>{filtradas.length} resultado{filtradas.length !== 1 ? 's' : ''}</span>
      </div>

      {/* Task list */}
      <div className="card" style={{ overflow: 'hidden' }}>
        {loading ? (
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}>Carregando...</div>
        ) : filtradas.length === 0 ? (
          ...
          return (
            <div key={t.id} style={{ borderTop: '1px solid var(--border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 16px', cursor: 'pointer', transition: 'background .1s' }} onClick={() => setAberta(open ? null : t.id)}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                  ...
                  <i className={`fa-solid ${open ? 'fa-chevron-up' : 'fa-chevron-down'}`} style={{ fontSize: 10, color: 'var(--text-dim)' }} />
                </div>
              </div>
              ...
              {open && (
                <div style={{ padding: '0 16px 16px' }}>
                  ...
                  {t.resultado ? (
                    <div style={{ padding: '14px 16px', background: 'var(--surface-2)', borderRadius: 8, border: '1px solid var(--border)', fontSize: 13, color: 'var(--text)', lineHeight: 1.75, whiteSpace: 'pre-wrap' }}>
                      {t.resultado}
                    </div>
                  ) : (
                    <div style={{ padding: '14px 16px', background: 'var(--surface-2)', borderRadius: 8, color: 'var(--text-muted)', fontSize: 12 }}>Aguardando resultado...</div>
                  )}
                  <div style={{ display: 'flex', gap: 16, marginTop: 10 }}>
                    ...
                    {t.completed_at && <span style={{ fontSize: 10.5, color: 'var(--text-dim)' }}>Concluido em {new Date(t.completed_at).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</span>}
                  </div>
                </div>
              )}
            </div>
