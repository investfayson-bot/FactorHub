-- ============================================================
-- FactorHub v2 — schema additions
-- ============================================================

-- ── hub_cerebro: new columns added in v2 ─────────────────────
ALTER TABLE public.hub_cerebro
  ADD COLUMN IF NOT EXISTS dna_fundador       text,
  ADD COLUMN IF NOT EXISTS knowledge_vault    text,
  ADD COLUMN IF NOT EXISTS playbooks          text,
  ADD COLUMN IF NOT EXISTS memoria_corporativa text,
  ADD COLUMN IF NOT EXISTS licoes_aprendidas  text;

-- ── hub_projetos: decision workflow ──────────────────────────
ALTER TABLE public.hub_projetos
  ADD COLUMN IF NOT EXISTS decisao      text CHECK (decisao IN ('aprovado', 'rejeitado')),
